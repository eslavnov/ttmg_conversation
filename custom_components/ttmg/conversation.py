...
from homeassistant.const import CONF_LLM_HASS_API, MATCH_ALL, CONF_URL
...
class OpenAIConversationEntity(
    ...
    async def async_process(
        ...
        # Return a default response intent immediately
        default_intent_response = intent.IntentResponse(language=user_input.language)
        default_intent_response.async_set_speech("Processing your request, please wait...")
        default_conversation_result = conversation.ConversationResult(
            response=default_intent_response, conversation_id=conversation_id
        )
          ...
          async with aiohttp.ClientSession() as session:
            ...
            async with session.post(f"{self.url}/preload/{user_input.device_id}", json={"messages": json.dumps(messages), "tools":json.dumps(tools), "max_completion_tokens":max_completion_tokens, "top_p":top_p, "temperature":temperature, "model": model  }) as resp:
                ...
                if tool_calls:
                    ...
                    async with session.get(f'{self.url}/history/{user_input.device_id}') as resp:
                        if resp.status != 200:
                            # Handle error
                            pass
                        response_body = await resp.json()
                    ...
                    async with session.post(f'{self.url}/write_history/{user_input.device_id}', json={"messages": new_messages }) as resp:
                      if resp.status != 200:
                          # Handle error
                          pass
                    self.history[conversation_id] = new_messages
        ...
        # Return the default response immediately
        return default_conversation_result
